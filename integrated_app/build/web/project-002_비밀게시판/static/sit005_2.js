...
function render(){const rows=filtered();tbody.innerHTML='';for(const r of rows){const tr=document.createElement('tr');tr.innerHTML=`<td><input type="checkbox"class="ckRow"data-key="${r.SEC}::${
          r.CD
        }"></td><td><span class="chip"title="Section">${escapeHtml(r.SEC)}</span></td><td>${escapeHtml(r.CD)}</td><td>${escapeHtml(r.NM)}</td><td style="text-align:right">${escapeHtml(r.NO??'')}</td><td>${escapeHtml(r.SECREF??'')}</td><td>${escapeHtml(r.SYSREF??'')}</td><td class="actions"><button class="btn"data-act="edit"data-key="${r.SEC}::${
          r.CD
        }">수정</button><button class="btn danger"data-act="del"data-key="${r.SEC}::${
          r.CD
        }">삭제</button></td>`;tbody.appendChild(tr);}
setCount();}
