...
async downloadAsImage(){const hexagramContainer=document.querySelector('.hexagram-container');if(!hexagramContainer){alert('❌ 괘 분석 영역을 찾을 수 없습니다.');return;}
try{await this.loadHtml2Canvas();const canvas=await html2canvas(hexagramContainer,{backgroundColor:'#ffffff',scale:2,logging:false,useCORS:true,});canvas.toBlob((blob)=>{const url=URL.createObjectURL(blob);const link=document.createElement('a');const now=new Date();const timestamp=now.toISOString().replace(/[:.]/g,'-').slice(0,-5);link.download=`육효분석_${timestamp}.png`;link.href=url;link.click();URL.revokeObjectURL(url);alert('✅ 이미지가 다운로드되었습니다!');});}catch(error){console.error('이미지 생성 실패:',error);alert('❌ 이미지 생성에 실패했습니다.\n페이지를 새로고침한 후 다시 시도해주세요.');}}
...
showShareOptions(htmlContent){const sectionHtmlContent=this.generateY6ShareSectionHtml();const textContent=this.generateY6ShareTextPlain();const modal=document.createElement('div');modal.style.cssText=`
    ...
    `;modal.appendChild(modalContent);document.body.appendChild(modal);document.getElementById('downloadImageBtn').addEventListener('click',()=>{this.downloadAsImage();document.body.removeChild(modal);});document.getElementById('copyTextBtn').addEventListener('click',()=>{this.copyToClipboard(textContent);document.body.removeChild(modal);});document.getElementById('viewHtmlBtn').addEventListener('click',()=>{const previewWindow=window.open('','_blank','width=800,height=600');const baseUrl=window.location.origin;previewWindow.document.write('<html><head><title>육효 분석 결과</title>');previewWindow.document.write('<meta charset="utf-8">');previewWindow.document.write(`<link rel="stylesheet" href="${baseUrl}/y6/static/y6_style.css">`);previewWindow.document.write(`
        <style>
