function toggleYundal(){const calendar=document.querySelector('input[name=__STRING_LITERAL_138__]:checked').value;const yundalSection=document.getElementById('yundal-section');if(calendar==='lunar'){yundalSection.style.display='block';}else{yundalSection.style.display='none';document.getElementById('yundal').checked=false;}}function toggleTimeInput(){const inputType=document.querySelector('input[name=__STRING_LITERAL_139__]:checked').value;const timeSection=document.getElementById('time-input-section');const ganjiSection=document.getElementById('ganji-input-section');if(inputType==='time'){timeSection.style.display='block';ganjiSection.style.display='none';}else{timeSection.style.display='none';ganjiSection.style.display='block';}}function getHourFromJiji(ji){const timeMap={子: 0,丑: 2,寅: 4,卯: 6,辰: 8,巳: 10,午: 12,未: 14,申: 16,酉: 18,戌: 20,亥: 22,미상:-1,};return timeMap[ji] !==undefined ? timeMap[ji] :-1;}function lunarToSolar(lunarYear,lunarMonth,lunarDay,isYundal){let solarYear=lunarYear;let solarMonth=lunarMonth+1;let solarDay=lunarDay;if(isYundal){solarMonth+=1;}if(solarMonth > 12){solarMonth-=12;solarYear+=1;}if(solarDay > 28){const daysInMonth=new Date(solarYear,solarMonth,0).getDate();if(solarDay > daysInMonth){solarDay=daysInMonth;}}return{year: solarYear,month: solarMonth,day: solarDay,warning:
 ...
 <div style="font-weight:700;font-size:1em;color:#2a5298;">${solarInfoText}</div>
    ...
    `;resultSection.insertAdjacentHTML('afterbegin',infoHTML);}function getElementClass(element){const map={木: 'wood',火: 'fire',土: 'earth',金: 'metal',水: 'water'};return map[element] || '';}function displayBasicSaju(saju,sibiun,yearGan,yearJi,dayGan,hap,hyungchung,sinsal,hyungchungEnhanced,sibiSinsal,gilsin,extraSinsal){const pillars=saju.hasHourInfo
 ? ['year','month','day','hour']
 : ['year','month','day'];const ganList=Object.values(saju.cheongan).filter((g)=> g !==null);const jiList=Object.values(saju.jiji).filter((j)=> j !==null);pillars.forEach((pillar,idx)=>{const gan=ganList[idx];const ji=jiList[idx];let ganSipseong='';let ganElement='';if(gan && gan !==dayGan){ganSipseong=getSipseongFromGanDetailed(dayGan,gan);ganElement=CHEONGAN_ELEMENT[gan];}else if(gan===dayGan){if(pillar==='day'){ganSipseong='日干';}else{ganSipseong='比肩';}ganElement=CHEONGAN_ELEMENT[gan];}let jiSipseong='';let jiElement='';if(ji && JIJANGGAN[ji]){const hongi=JIJANGGAN[ji].find((j)=> j.type==='본기' || j.type==='정기');if(hongi){const mainGan=hongi.gan;jiElement=JIJI_ELEMENT[ji];if(mainGan !==dayGan){jiSipseong=getSipseongFromGanDetailed(dayGan,mainGan);}else{jiSipseong='比肩';}}}const ganLabel=ganSipseong ? `${ganSipseong}(${ganElement})` : '';const jiLabel=jiSipseong ? `${jiSipseong}(${jiElement})` : '';const ganEl=document.getElementById(`${pillar}-gan`);if(ganEl && gan){ganEl.innerHTML=`
            ...
            `;ganEl.className='gan-box '+getElementClass(CHEONGAN_ELEMENT[gan]);}const jiEl=document.getElementById(`${pillar}-ji`);if(jiEl && ji){jiEl.innerHTML=`
            ...
            `;jiEl.className='ji-box '+getElementClass(JIJI_ELEMENT[ji]);}const sibiunEl=document.getElementById(`${pillar}-sibiun`);if(sibiunEl && sibiun[pillar]){sibiunEl.textContent=sibiun[pillar];}const napeumEl=document.getElementById(`${pillar}-napeum`);if(napeumEl && gan && ji){const napeum=getNapeum(gan,ji);napeumEl.textContent=napeum.full;}const gongmangEl=document.getElementById(`${pillar}-gongmang`);if(gongmangEl && ji){const isGongmang=getGongmang(yearGan,yearJi,ji);gongmangEl.textContent=isGongmang ? '⚠️ 공망' : '✓';gongmangEl.className=isGongmang
 ? 'gongmang-info'
 : 'gongmang-info empty';}});if(!saju.hasHourInfo){const hourCard=document.querySelector('.pillar-card:first-child');if(hourCard){hourCard.style.opacity='0.3';hourCard.style.pointerEvents='none';const hourTitle=hourCard.querySelector('.pillar-title');if(hourTitle){hourTitle.innerHTML='時柱 (시주)<br><small style=__STRING_LITERAL_152__>시각 미상</small>';}}}const elementCount={木: 0,火: 0,土: 0,金: 0,水: 0};ganList.forEach((gan)=>{if(gan){const element=CHEONGAN_ELEMENT[gan];elementCount[element]++;}});jiList.forEach((ji)=>{if(ji){const element=JIJI_ELEMENT[ji];elementCount[element]++;}});const elementSummary=Object.keys(elementCount).map((el)=> `${el}(${elementCount[el]})`).join(', ');const pillarNames={year: '년주',month: '월주',day: '일주',hour: '시주',};const pillarOrder=['hour','day','month','year'];let jijangganSummary='';pillarOrder.forEach((pillar)=>{const idx=pillars.indexOf(pillar);if(idx !==-1){const ji=jiList[idx];if(ji && JIJANGGAN[ji]){const jijangganList=JIJANGGAN[ji];const ganStr=jijangganList.map((item)=> item.gan).join('');jijangganSummary+=`${pillarNames[pillar]}: ${ganStr}　`;}}});const summaryEl=document.getElementById('pillar-element-summary');if(summaryEl){let additionalInfo='';additionalInfo+=`<div id="element-summary-header" style="margin-bottom:8px;cursor:pointer;padding:5px;border-radius:5px;transition:background 0.2s;" onmouseover="this.style.background='#e3f2fd'" onmouseout="this.style.background='transparent'" onclick="document.getElementById('element-summary-details').style.display = document.getElementById('element-summary-details').style.display === 'none' ? 'block' : 'none';">
      <span style="color:#666;">🔍 오행:</span> ${elementSummary} <span style="color:#999;font-size:0.8em;">(클릭:神殺보기)</span>
    </div>`;additionalInfo+=`<div id="element-summary-details" style="display:none;padding-left:10px;border-left:3px solid #e0e0e0;">`;additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;color:#888;"><span style="color:#666;">지장간:</span> ${jijangganSummary}</div>`;if(hap){let hapSummary='';let hapDetails=[];if(hap.cheonganHap && hap.cheonganHap.length > 0){hap.cheonganHap.forEach((h)=>{hapDetails.push(`${h.pair} → ${h.result}`);});}if(hap.jijiHap && hap.jijiHap.length > 0){hap.jijiHap.forEach((h)=>{hapDetails.push(`${h.name}(${h.pair})`);});}if(hap.samhap && hap.samhap.length > 0){hap.samhap.forEach((s)=>{if(s.pattern){const completeText=s.complete ? '완성' : '반합';hapDetails.push(`${s.type || '삼합'}(${s.pattern}) ${completeText}`);}});}if(hapDetails.length > 0){hapSummary=hapDetails.join(', ');additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;"><span style="color:#666;">合 (합):</span> <span style="color:#4caf50;">${hapSummary}</span></div>`;}else{additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;color:#999;"><span style="color:#666;">合 (합):</span> 없음</div>`;}}if(hyungchungEnhanced){if(hyungchungEnhanced.chung.length===0 &&
 hyungchungEnhanced.hyung.length===0 &&
 hyungchungEnhanced.pa.length===0 &&
 hyungchungEnhanced.hae.length===0 &&
 hyungchungEnhanced.wongjin.length===0){additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;color:#999;"><span style="color:#666;">刑沖破害:</span> 없음 (안정적)</div>`;}else{const items=[];if(hyungchungEnhanced.chung.length > 0){const chungItems=hyungchungEnhanced.chung
 .map((c)=> c.name).join(', ');items.push(`<span style="color:#d32f2f;">충(沖): ${chungItems}</span>`);}if(hyungchungEnhanced.hyung.length > 0){const hyungItems=hyungchungEnhanced.hyung
 .map((h)=> h.name).join(', ');items.push(`<span style="color:#c62828;">형(刑): ${hyungItems}</span>`);}if(hyungchungEnhanced.pa.length > 0){const paItems=hyungchungEnhanced.pa.map((p)=> p.name).join(', ');items.push(`<span style="color:#e64a19;">파(破): ${paItems}</span>`);}if(hyungchungEnhanced.hae.length > 0){const haeItems=hyungchungEnhanced.hae.map((h)=> h.name).join(', ');items.push(`<span style="color:#f57c00;">해(害): ${haeItems}</span>`);}if(hyungchungEnhanced.wongjin.length > 0){const wonjinItems=hyungchungEnhanced.wongjin
 .map((w)=> w.pair).join(', ');items.push(`<span style="color:#5d4037;">원진(怨嗔): ${wonjinItems}</span>`);}if(items.length > 0){additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;"><span style="color:#666;">刑沖破害:</span> ${items.join(
            ' / '
          )}</div>`;}}}if(sinsal){let sinsalSummary='';if(sinsal.good.length > 0){sinsalSummary+='길신: '+sinsal.good.map((s)=> s.name).join(', ');}if(sinsal.bad.length > 0){if(sinsalSummary)sinsalSummary+=' / ';sinsalSummary+='흉신: '+sinsal.bad.map((s)=> s.name).join(', ');}if(sinsalSummary){additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;"><span style="color:#666;">神殺:</span> ${sinsalSummary}</div>`;}}const gongmangJi=[];jiList.forEach((ji)=>{if(ji){const isGongmang=getGongmang(yearGan,yearJi,ji);if(isGongmang){gongmangJi.push(ji);}}});if(gongmangJi.length > 0){additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;"><span style="color:#666;">空亡:</span> <span style="color:#ff9800;">${gongmangJi.join(
        ', '
      )}</span></div>`;}else{additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;color:#999;"><span style="color:#666;">空亡:</span> 없음</div>`;}if(sinsal){const specialSinsal=[];sinsal.good.forEach((s)=>{if(['天乙貴人','學堂貴人'].includes(s.name)){specialSinsal.push(s.name);}});sinsal.bad.forEach((s)=>{if(['桃花殺','驛馬殺','三災','白虎大殺'].includes(s.name)){specialSinsal.push(s.name);}});if(specialSinsal.length > 0){additionalInfo+=`<div style="margin-bottom:8px;font-size:0.9em;"><span style="color:#666;">특수 神殺:</span> ${specialSinsal.join(
          ', '
        )}</div>`;}}if(hyungchungEnhanced){additionalInfo+=generateHyungChungSummary(hyungchungEnhanced);}if(sibiSinsal && sibiSinsal.length > 0){additionalInfo+=generateSibiSinsalSummary(sibiSinsal);}if(gilsin && gilsin.length > 0){additionalInfo+=generateGilsinSummary(gilsin);}if(extraSinsal && extraSinsal.length > 0){additionalInfo+=generateExtraSinsalSummary(extraSinsal);}additionalInfo+=`</div>`;summaryEl.innerHTML=additionalInfo;}}function displayDayMaster(dayGan){const el=CHEONGAN_ELEMENT[dayGan];const html=`
    ...
    `;document.getElementById('daymaster-analysis').innerHTML=html;}function displayStrength(strength){const html=`
    ...
    `;document.getElementById('strength-analysis').innerHTML=html;}function displayGyeokguk(gyeokguk){const html=`
        <div class="gyeokguk-result">
            <div class="gyeokguk-name">${gyeokguk.name}</div>
            <div class="gyeokguk-desc">${gyeokguk.desc}</div>
        </div>
    `;document.getElementById('gyeokguk-analysis').innerHTML=html;}function displayYongsin(yongsin){const html=`
    ...
    `;document.getElementById('yongsin-analysis').innerHTML=html;}function displayElements(elements){const total=Object.values(elements).reduce((a,b)=> a+b,0);const names={木: '木(목)',火: '火(화)',土: '土(토)',金: '金(금)',水: '水(수)',};let chartHTML='';Object.keys(elements).forEach((el)=>{const count=elements[el].toFixed(1);const pct=total > 0 ?((elements[el]/total)*100).toFixed(1): 0;chartHTML+=`
 ...
 elements[a] < elements[b] ? a : b);const summaryHTML=`
    ...
    `;document.getElementById('element-chart').innerHTML=chartHTML;document.getElementById('element-summary').innerHTML=summaryHTML;}function displayYinYang(yinyang){const total=yinyang.yang+yinyang.yin;const yangPct=((yinyang.yang/total)*100).toFixed(1);const chartHTML=`
    ...
    100 - yangPct
  ).toFixed(1)}%<br><br>`;if(yinyang.yang > yinyang.yin+2){summary+='양의 기운이 강하여 적극적, 활동적, 외향적입니다. 리더십과 추진력이 뛰어나나 성급함을 경계해야 합니다.';}else if(yinyang.yin > yinyang.yang+2){summary+='음의 기운이 강하여 차분하고 내향적이며 사려 깊습니다. 인내심과 포용력이 뛰어나나 소극적이지 않도록 주의해야 합니다.';}else{summary+='음양의 균형이 잘 잡혀 있습니다. 상황에 따라 적극적이거나 신중할 수 있으며 조화로운 성격입니다.';}document.getElementById('yinyang-chart').innerHTML=chartHTML;document.getElementById('yinyang-summary').innerHTML=summary;}function displayJijanggan(saju){const pillars=['hour','day','month','year'];const names=['시지','일지','월지','년지'];let html='';pillars.forEach((p,idx)=>{const ji=saju.jiji[p];if(ji && JIJANGGAN[ji]){const list=JIJANGGAN[ji];let ganHTML='';list.forEach((item)=>{ganHTML+=`
            ...
            `;}});itemHTML+=`</div>`;}});const maxStar=Object.keys(sipseongDetailed).reduce((a,b)=>
 ...
 sipseong[a] > sipseong[b] ? a : b);const summary=`
    ...
    `;document.getElementById('sibiun-detail-analysis').innerHTML=gridHTML+summary;}function displayYukchin(yukchin){let gridHTML='';Object.entries(yukchin).forEach(([relation,data])=>{gridHTML+=`
    ...
    `;document.getElementById('yukchin-analysis').innerHTML=gridHTML+summary;}function displayHap(hap){if(hap.cheonganHap.length===0 &&
 hap.jijiHap.length===0 &&
 hap.samhap.length===0){document.getElementById('hap-analysis').innerHTML='<div class=__STRING_LITERAL_153__>사주에 특별한 합이 없습니다</div>';return;}let html='';if(hap.cheonganHap.length > 0){html+=`
        ...
        `;}document.getElementById('hap-analysis').innerHTML=html;}function displayHyungChungEnhanced(hyungchung){const container=document.getElementById('hyungchung-analysis');if(hyungchung.chung.length===0 &&
 ...
 <div style="font-size:1.2em;font-weight:700;color:#b71c1c;">${c.name}</div>
 <div style="background:${
                c.severity === '중요' ? '#d32f2f' : '#ff9800'
              };color:white;padding:4px 12px;border-radius:20px;font-size:0.85em;">
 ${c.severity}</div>
 </div>
 <div style="font-size:0.95em;color:#555;margin-bottom:8px;">${c.desc}</div>
 ${c.impact
 ...
 .map((p)=>{const names={year: '년주',month: '월주',day: '일주',hour: '시주',};return names[p];}).join(', ')}</div>
 </div>
 `
          )
          .join('')}
 ...
 <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
 <div style="font-size:1.2em;font-weight:700;color:#b71c1c;">${h.name}</div>
 <div style="background:${
                h.severity === '심각' ? '#c62828' : '#ff6f00'
              };color:white;padding:4px 12px;border-radius:20px;font-size:0.85em;">
 ${h.severity}</div>
 </div>
 <div style="font-size:0.95em;color:#555;margin-bottom:8px;">${h.type}: ${h.desc}</div>
 <div style="background:#fff;padding:10px;border-radius:8px;color:#c62828;font-weight:600;">
 ${h.complete ? '🔴 완전형성' : '🟡 부분형성'}-${h.found}</div>
 </div>
 `
          )
          .join('')}
 ...
 `
    ...
    `;}container.innerHTML=html;}function displayHyungChung(hyungchung){displayHyungChungEnhanced(hyungchung);}function displaySinsal(sinsal){let html='<div class=__STRING_LITERAL_180__>';sinsal.good.forEach((s)=>{html+=`
 ...
 sinsal.bad.length===0 &&
 sinsal.special.length===0){html='<div style=__STRING_LITERAL_181__>사주에 특별한 神殺(신살)이 확인되지 않았습니다</div>';}document.getElementById('sinsal-analysis').innerHTML=html;}function displayGongmangDetail(saju,yearGan,yearJi){const jijiList=Object.values(saju.jiji);const pillars=['年支','月支','日支','時支'];let html='<div style=__STRING_LITERAL_182__>';html+='<p><strong>▶ 空亡(공망)이란?</strong> 60갑자 순환에서 빠진 지지로, 그 위치의 기운이 약하거나 공허함을 의미합니다.</p><br>';html+='<p><strong>▶ 각 기둥의 空亡(공망) 여부:</strong></p><ul style=__STRING_LITERAL_183__>';jijiList.forEach((ji,idx)=>{const isGongmang=getGongmang(yearGan,yearJi,ji);html+=`<li style="padding:8px 0;">${pillars[idx]} ${ji}: ${
      isGongmang
        ? '<strong style="color:#c62828;">空亡 ⚠️</strong> - 이 분야가 약하거나 공허할 수 있습니다'
        : '<strong style="color:#2e7d32;">정상 ✓</strong>'
    }</li>`;});html+='</ul><br>';html+='<p><strong>▶ 대처법:</strong> 空亡(공망)이 있는 기둥은 노력으로 보완해야 합니다. 해당 육친이나 분야에 더욱 신경 쓰고 준비하세요.</p>';html+='</div>';document.getElementById('gongmang-detail-analysis').innerHTML=html;}function displaySpecialSinsal(sinsal){let html='<div style=__STRING_LITERAL_184__>';const hasSpecial=sinsal.good.some((s)=> ['天乙貴人','學堂貴人'].includes(s.name))||
 sinsal.bad.some((s)=>
 ['桃花殺','驛馬殺','三災','白虎大殺'].includes(s.name));if(!hasSpecial){html+='<p style=__STRING_LITERAL_185__>특별히 주목할 만한 神殺(신살)이 없습니다</p>';}else{html+='<p><strong>▶ 주요 神殺(신살) 상세 해석:</strong></p><br>';sinsal.good.forEach((s)=>{if(['天乙貴人','學堂貴人'].includes(s.name)){html+=`<p style="margin-bottom:15px;">• <strong>${s.name}:</strong> ${s.desc}</p>`;}});sinsal.bad.forEach((s)=>{if(['桃花殺','驛馬殺','三災','白虎大殺'].includes(s.name)){html+=`<p style="margin-bottom:15px;">• <strong>${s.name}:</strong> ${s.desc}</p>`;}});}html+='</div>';document.getElementById('special-sinsal-analysis').innerHTML=html;}function displayDaeun(daeunList,dayGan,birthYear){let itemsHTML='';const reversedDaeun=[...daeunList].reverse();reversedDaeun.forEach((daeun)=>{const ganEl=CHEONGAN_ELEMENT[daeun.gan];const jiEl=JIJI_ELEMENT[daeun.jiji];const daeunStartYear=birthYear+daeun.age;itemsHTML+=`
    ...
    `;const summary=``;document.getElementById('daeun-chart').innerHTML=chartHTML;document.getElementById('daeun-summary').innerHTML=summary;setTimeout(()=>{const daeunScroll=document.querySelector('.daeun-scroll');if(daeunScroll){const currentYear=new Date().getFullYear();const currentAge=currentYear-birthYear;const currentDaeun=daeunList.find((d)=> currentAge >=d.age && currentAge <=d.endAge);if(currentDaeun){const daeunItems=document.querySelectorAll('.daeun-item');let targetItem=null;daeunItems.forEach((item)=>{const title=item.getAttribute('title');if(title && title.includes(`만 ${currentDaeun.age}세`)){targetItem=item;}});if(targetItem){const itemLeft=targetItem.offsetLeft;const itemWidth=targetItem.offsetWidth;const scrollWidth=daeunScroll.offsetWidth;const targetScroll=itemLeft-scrollWidth/2+itemWidth/2;daeunScroll.scrollTo({left: targetScroll,behavior: 'smooth'});targetItem.style.background='linear-gradient(135deg, #fff9e6 0%, #fff3cd 100%)';targetItem.style.border='3px solid #ffc107';targetItem.style.boxShadow='0 4px 12px rgba(255, 193, 7, 0.3)';}}else{const scrollWidth=daeunScroll.scrollWidth;const clientWidth=daeunScroll.clientWidth;const centerScroll=(scrollWidth-clientWidth)/2;daeunScroll.scrollTo({left: centerScroll,behavior: 'smooth'});}}},100);}function displayYeonun(yeonunList){const filteredYeonun=yeonunList.filter((yeonun)=> yeonun.age >=1);let chartHTML='<div class=__STRING_LITERAL_186__ style=__STRING_LITERAL_187__><h3 style=__STRING_LITERAL_188__>📅 年運 (연운) - 60甲子 年柱 <span style=__STRING_LITERAL_189__>(클릭하여 월운 보기)</span></h3><div class=__STRING_LITERAL_190__ style=__STRING_LITERAL_191__>';filteredYeonun.forEach((yeonun)=>{const ganEl=CHEONGAN_ELEMENT[yeonun.gan];const jiEl=JIJI_ELEMENT[yeonun.ji];chartHTML+=`
      ...
      yeonun.ji
        ...
        `;});chartHTML+='</div></div>';return chartHTML;}function displayWolun(wolunList){let chartHTML='<div class=__STRING_LITERAL_192__ id=__STRING_LITERAL_193__ style=__STRING_LITERAL_194__><h3 style=__STRING_LITERAL_195__>📆 月運 (월운) - 前年·現在年·次年 月柱 (36개월)</h3>';const monthNames=[
 '1月','2月','3月','4月','5月','6月','7月','8月','9月','10月','11月','12月',];const years=[...new Set(wolunList.map((w)=> w.year))].sort((a,b)=> a-b);if(years.length===0){chartHTML+='<div style=__STRING_LITERAL_196__>월운 데이터가 없습니다.</div></div>';return chartHTML;}const prevYear=years[0];const currentYear=years[1] || prevYear;const nextYear=years[2] || years[1] || prevYear;chartHTML+=`
    ...
    `;chartHTML+='<div class=__STRING_LITERAL_197__ style=__STRING_LITERAL_198__>';[nextYear,currentYear,prevYear].forEach((year,yearIdx)=>{const yearLabel=yearIdx===0 ? '익년도' : yearIdx===1 ? '당년도' : '전년도';const yearColor=yearIdx===0 ? '#ff9800' : yearIdx===1 ? '#2196f3' : '#4caf50';for(let month=12;month >=1;month--){const wolun=wolunList.find((w)=> w.year===year && w.month===month);if(wolun){const ganEl=CHEONGAN_ELEMENT[wolun.gan];const jiEl=JIJI_ELEMENT[wolun.zhi];chartHTML+=`
          ...
          wolun.zhi
            ...
            `;}});chartHTML+='</div></div>';setTimeout(()=>{const wolunScroll=document.querySelector('.wolun-scroll');if(wolunScroll){const currentYearItems=document.querySelectorAll(`.wolun-month-item[data-year="${currentYear}"]`);if(currentYearItems.length > 0){const currentMonth=new Date().getMonth()+1;let targetItem=Array.from(currentYearItems).find((item)=> parseInt(item.getAttribute('data-month'))===currentMonth);if(!targetItem){targetItem=Array.from(currentYearItems).find((item)=> parseInt(item.getAttribute('data-month'))===6);}if(!targetItem && currentYearItems.length > 0){targetItem=currentYearItems[Math.floor(currentYearItems.length/2)];}if(targetItem){const itemLeft=targetItem.offsetLeft;const itemWidth=targetItem.offsetWidth;const scrollWidth=wolunScroll.offsetWidth;const targetScroll=itemLeft-scrollWidth/2+itemWidth/2;wolunScroll.scrollLeft=targetScroll;}}}},50);return chartHTML;}function displaySewun(sewun,dayGan){const ganEl=CHEONGAN_ELEMENT[sewun.gan];const jiEl=JIJI_ELEMENT[sewun.ji];const html=`
    ...
    `;document.getElementById('sewun-analysis').innerHTML=html;}function displayComprehensive(saju,dayGan,strength,gyeokguk,yongsin,elements,sibiun){const dayEl=CHEONGAN_ELEMENT[dayGan];const maxEl=Object.keys(elements).reduce((a,b)=>
 elements[a] > elements[b] ? a : b);const html=`
    ...
    `;document.getElementById('gaewun-guide').innerHTML=html;}function scrollToYeonun(year){const yeonunItem=document.querySelector(`.yeonun-item[data-year="${year}"]`);if(yeonunItem){document.querySelectorAll('.yeonun-item').forEach((item)=>{item.style.background='#f8f9fa';item.style.transform='scale(1)';item.style.boxShadow='none';});yeonunItem.style.background='linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%)';yeonunItem.style.transform='scale(1.1)';yeonunItem.style.boxShadow='0 4px 12px rgba(255, 152, 0, 0.3)';yeonunItem.style.border='2px solid #ff9800';const yeonunScroll=yeonunItem.closest('.yeonun-scroll');if(yeonunScroll){const itemLeft=yeonunItem.offsetLeft;const itemWidth=yeonunItem.offsetWidth;const scrollLeft=yeonunScroll.scrollLeft;const scrollWidth=yeonunScroll.offsetWidth;const targetScroll=itemLeft-scrollWidth/2+itemWidth/2;yeonunScroll.scrollTo({left: targetScroll,behavior: 'smooth'});}setTimeout(()=>{autoTriggerWolun(year);},200);}}function scrollToWolun(year){console.log('🔍 scrollToWolun 호출 - 연도:',year);document.querySelectorAll('.yeonun-item').forEach((item)=>{item.style.background='#f8f9fa';item.style.transform='scale(1)';item.style.boxShadow='none';item.style.border='none';});SajuAPI.calcWolun(year).then((wolunList)=>{console.log('📊 scrollToWolun - Python API 월운 응답:',wolunList);if(wolunList && wolunList.length > 0){console.log('📊 scrollToWolun - 첫 월운 gan:',wolunList[0].gan,'zhi:',wolunList[0].zhi);}const wolunHTML=displayWolun(wolunList);const existingWolunContainer=document.getElementById('wolun-container');if(existingWolunContainer){const tempDiv=document.createElement('div');tempDiv.innerHTML=wolunHTML;const newWolunContainer=tempDiv.firstChild;existingWolunContainer.parentNode.replaceChild(newWolunContainer,existingWolunContainer);}else{const yeonunContainer=document.querySelector('.yeonun-container');if(yeonunContainer){yeonunContainer.insertAdjacentHTML('afterend',wolunHTML);}}setTimeout(()=>{const wolunItems=document.querySelectorAll(`.wolun-month-item[data-year="${year}"]`);if(wolunItems.length > 0){wolunItems.forEach((item)=>{item.style.background='linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%)';item.style.transform='scale(1.1)';item.style.boxShadow='0 6px 16px rgba(255, 152, 0, 0.4)';item.style.border='3px solid #ff9800';});const currentMonth=new Date().getMonth()+1;let targetMonthItem=Array.from(wolunItems).find((item)=> parseInt(item.getAttribute('data-month'))===currentMonth);if(!targetMonthItem){targetMonthItem=Array.from(wolunItems).find((item)=> parseInt(item.getAttribute('data-month'))===6);}if(!targetMonthItem && wolunItems.length > 0){targetMonthItem=wolunItems[Math.floor(wolunItems.length/2)];}if(targetMonthItem){const wolunScroll=targetMonthItem.closest('.wolun-scroll');if(wolunScroll){const itemLeft=targetMonthItem.offsetLeft;const itemWidth=targetMonthItem.offsetWidth;const scrollWidth=wolunScroll.offsetWidth;const targetScroll=itemLeft-scrollWidth/2+itemWidth/2;wolunScroll.scrollTo({left: targetScroll,behavior: 'smooth'});}}}},300);}).catch((error)=>{console.error('❌ scrollToWolun 월운 계산 오류:',error);alert('월운 계산 중 오류가 발생했습니다.');});}function autoTriggerDaeun(birthYear,daeunList){const currentYear=new Date().getFullYear();const currentAge=currentYear-birthYear;console.log('🎯 autoTriggerDaeun 호출 - birthYear:',birthYear,'currentYear:',currentYear,'currentAge:',currentAge);const currentDaeun=daeunList.find((daeun)=> currentAge >=daeun.age && currentAge <=daeun.endAge);if(currentDaeun){console.log('✅ 현재 대운 찾음:',currentDaeun);setTimeout(()=>{console.log('⏰ autoTriggerDaeun setTimeout 실행 (3초 후)');const daeunItems=document.querySelectorAll('.daeun-item');daeunItems.forEach((item)=>{const ageText=item.querySelector('.daeun-age').textContent;const ageRange=ageText
 .split('-').map((a)=> parseInt(a.replace('세','')));if(currentAge >=ageRange[0] && currentAge <=ageRange[1]){console.log('✅ 대운 하이라이트:',ageRange[0],'-',ageRange[1]);item.style.background='linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%)';item.style.transform='scale(1.1)';item.style.boxShadow='0 8px 16px rgba(255, 152, 0, 0.4)';item.style.border='3px solid #ff9800';const daeunScroll=item.closest('.daeun-scroll');if(daeunScroll){const itemLeft=item.offsetLeft;const itemWidth=item.offsetWidth;const scrollWidth=daeunScroll.offsetWidth;const targetScroll=itemLeft-scrollWidth/2+itemWidth/2;daeunScroll.scrollTo({left: targetScroll,behavior: 'smooth'});}const daeunStartYear=birthYear+ageRange[0];console.log('🔄 연운 트리거 예약 - daeunStartYear:',daeunStartYear,'currentYear:',currentYear);setTimeout(()=>{console.log('▶️ 연운 트리거 실행!');autoTriggerYeonun(daeunStartYear,currentYear);},1500);}});},3000);}else{console.log('❌ 현재 대운을 찾지 못함');}}function autoTriggerYeonun(daeunStartYear,targetYear){console.log('🔍 autoTriggerYeonun 호출됨 - targetYear:',targetYear);let attempts=0;const maxAttempts=50;const checkAndExecute=()=>{attempts++;console.log('🔄 연운 항목 확인 시도:',attempts);const yeonunItems=document.querySelectorAll('.yeonun-item');console.log('📊 연운 항목 개수:',yeonunItems.length);if(yeonunItems.length >=10){console.log('✅ 연운 항목 준비 완료! 실행 시작');yeonunItems.forEach((item)=>{item.style.background='#f8f9fa';item.style.transform='scale(1)';item.style.boxShadow='none';item.style.border='none';});let targetYeonunItem=null;yeonunItems.forEach((item)=>{const year=parseInt(item.getAttribute('data-year'));if(year===targetYear){console.log('✅ 목표 연도 찾음:',year);targetYeonunItem=item;item.style.background='linear-gradient(135deg, #fff3e0 0%, #ffe0b2 100%)';item.style.transform='scale(1.1)';item.style.boxShadow='0 6px 16px rgba(255, 152, 0, 0.4)';item.style.border='3px solid #ff9800';}});if(!targetYeonunItem){console.log('❌ 목표 연도를 찾지 못함:',targetYear);}if(targetYeonunItem){const yeonunScroll=targetYeonunItem.closest('.yeonun-scroll');if(yeonunScroll){const itemLeft=targetYeonunItem.offsetLeft;const itemWidth=targetYeonunItem.offsetWidth;const scrollWidth=yeonunScroll.offsetWidth;const targetScroll=itemLeft-scrollWidth/2+itemWidth/2;console.log('📍 중앙 스크롤 실행:',targetScroll);yeonunScroll.scrollTo({left: targetScroll,behavior: 'smooth'});}setTimeout(()=>{console.log('🌙 월운 자동 트리거 실행');autoTriggerWolun(targetYear);},500);}}else if(attempts < maxAttempts){console.log('⏳ 연운 항목 준비 중... 200ms 후 재시도');setTimeout(checkAndExecute,200);}else{console.log('❌ 연운 항목 로드 실패 (타임아웃)');}};setTimeout(checkAndExecute,500);}function autoTriggerWolun(year){console.log('🔍 autoTriggerWolun 호출 - 연도:',year);SajuAPI.calcWolun(year).then((wolunList)=>{console.log('📊 Python API 월운 응답:',wolunList);console.log('📊 첫 번째 월운:',wolunList[0]);if(wolunList && wolunList.length > 0){console.log('📊 첫 월운 gan:',wolunList[0].gan,'zhi:',wolunList[0].zhi);}const wolunHTML=displayWolun(wolunList);const existingWolunContainer=document.getElementById('wolun-container');if(existingWolunContainer){const tempDiv=document.createElement('div');tempDiv.innerHTML=wolunHTML;const newWolunContainer=tempDiv.firstChild;existingWolunContainer.parentNode.replaceChild(newWolunContainer,existingWolunContainer);}setTimeout(()=>{const wolunItems=document.querySelectorAll(`.wolun-month-item[data-year="${year}"]`);if(wolunItems.length > 0){wolunItems.forEach((item)=>{item.style.background='linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%)';item.style.transform='scale(1.05)';item.style.boxShadow='0 4px 12px rgba(33, 150, 243, 0.3)';item.style.border='3px solid #2196f3';});const currentMonth=new Date().getMonth()+1;let targetMonthItem=Array.from(wolunItems).find((item)=> parseInt(item.getAttribute('data-month'))===currentMonth);if(!targetMonthItem){targetMonthItem=Array.from(wolunItems).find((item)=> parseInt(item.getAttribute('data-month'))===6);}if(targetMonthItem){const wolunScroll=targetMonthItem.closest('.wolun-scroll');if(wolunScroll){const itemLeft=targetMonthItem.offsetLeft;const itemWidth=targetMonthItem.offsetWidth;const scrollWidth=wolunScroll.offsetWidth;const targetScroll=itemLeft-scrollWidth/2+itemWidth/2;wolunScroll.scrollTo({left: targetScroll,behavior: 'smooth'});}}}},300);}).catch((error)=>{console.error('월운 계산 오류:',error);alert('월운 계산 중 오류가 발생했습니다.');});}document.addEventListener('DOMContentLoaded',function(){console.log('✅ DOM 로드 완료 - 이벤트 리스너 등록');const analyzeBtn=document.getElementById('analyze-btn');if(analyzeBtn){analyzeBtn.addEventListener('click',analyzeSaju);console.log('✅ 분석 버튼 이벤트 등록 완료');}const timeInputRadios=document.querySelectorAll('input[name=__STRING_LITERAL_199__]');timeInputRadios.forEach((radio)=>{radio.addEventListener('change',toggleTimeInput);});console.log('✅ 시각 입력 방식 라디오 버튼 이벤트 등록 완료');toggleTimeInput();console.log('✅ 초기 시각 입력 방식 UI 설정 완료');const calendarRadios=document.querySelectorAll('input[name=__STRING_LITERAL_200__]');calendarRadios.forEach((radio)=>{radio.addEventListener('change',toggleYundal);});console.log('✅ 역법 라디오 버튼 이벤트 등록 완료');});function generateHyungChungSummary(hyungchungEnhanced){return '';}function generateSibiSinsalSummary(sibiSinsal){const goodSinsal=sibiSinsal.filter((s)=> s.type==='good');const badSinsal=sibiSinsal.filter((s)=> s.type==='bad');let html='<div style=__STRING_LITERAL_201__>';const items=[];if(goodSinsal.length > 0){items.push(`<span style="color:#4caf50;">길신: ${goodSinsal
        .map((s) => s.name)
        .join(', ')}</span>`);}if(badSinsal.length > 0){items.push(`<span style="color:#f44336;">흉신: ${badSinsal
        .map((s) => s.name)
        .join(', ')}</span>`);}if(items.length > 0){html+=`<div style="font-weight:700;color:#5e35b1;font-size:0.9em;">十二神殺: <span style="font-size:0.9em;font-weight:400;">${items.join(
      ' / '
    )}</span></div>`;}html+='</div>';return html;}function generateGilsinSummary(gilsin){let html='<div style=__STRING_LITERAL_202__>';const names=gilsin.map((g)=> g.name).join(', ');html+=`<div style="font-weight:700;color:#f57f17;font-size:0.9em;">吉神類: <span style="color:#f9a825;font-size:0.9em;font-weight:400;">${names}</span></div>`;html+='</div>';return html;}function generateExtraSinsalSummary(extraSinsal){const goodSinsal=extraSinsal.filter((s)=> s.type==='good');const warningSinsal=extraSinsal.filter((s)=> s.type==='warning');const badSinsal=extraSinsal.filter((s)=> s.type==='bad');let html='<div style=__STRING_LITERAL_203__>';const items=[];if(goodSinsal.length > 0){items.push(`<span style="color:#4caf50;">긍정: ${goodSinsal
        .map((s) => s.name)
        .join(', ')}</span>`);}if(warningSinsal.length > 0){items.push(`<span style="color:#ff9800;">주의: ${warningSinsal
        .map((s) => s.name)
        .join(', ')}</span>`);}if(badSinsal.length > 0){items.push(`<span style="color:#f44336;">부정: ${badSinsal
        .map((s) => s.name)
        .join(', ')}</span>`);}if(items.length > 0){html+=`<div style="font-weight:700;color:#0288d1;font-size:0.9em;">其他神殺: <span style="font-size:0.9em;font-weight:400;">${items.join(
      ' / '
    )}</span></div>`;}html+='</div>';return html;}function displaySibiSinsal(sibiSinsal){const container=document.getElementById('sibi-sinsal-analysis');if(!sibiSinsal || sibiSinsal.length===0){container.innerHTML=`
    ...
    `;return;}const goodSinsal=sibiSinsal.filter((s)=> s.type==='good');const badSinsal=sibiSinsal.filter((s)=> s.type==='bad');let html=`
 ...
 `
    ...
    `;}container.innerHTML=html;}function displayGilsin(gilsin){const container=document.getElementById('gilsin-analysis');if(!gilsin || gilsin.length===0){container.innerHTML=`
      <div style="text-align:center;padding:30px;background:#f5f5f5;border-radius:12px;">
 ...
 `
  ...
  `;container.innerHTML=html;}function displayExtraSinsal(extraSinsal){const container=document.getElementById('extra-sinsal-analysis');if(!extraSinsal || extraSinsal.length===0){container.innerHTML=`
      <div style="text-align:center;padding:30px;background:#f5f5f5;border-radius:12px;">
        <div style="font-size:1.1em;color:#666;">사주에 추가 신살이 없습니다.</div>
      </div>
    `;return;}const goodSinsal=extraSinsal.filter((s)=> s.type==='good');const warningSinsal=extraSinsal.filter((s)=> s.type==='warning');const badSinsal=extraSinsal.filter((s)=> s.type==='bad');let html=`
