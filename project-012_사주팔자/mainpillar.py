...
def convert_lunar_to_solar(year, month, day, is_leap):
    ...
    try:
        ...
        lunar = Lunar(int(year), int(month), int(day), isleap=is_leap)
        ...
        # 존재하지 않는 윤달 등의 경우
        if is_leap:
            return {'error': f'{year}년 {month}월에는 윤달이 없습니다. 윤달 체크를 해제하고 다시 시도해주세요.'}
        else:
            return {'error': f'유효하지 않은 음력 날짜입니다: {year}년 {month}월 {day}일'}
    except Exception as e:
