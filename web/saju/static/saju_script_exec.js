...
const startGan = [2, 4, 6, 8, 0][yearGroup];
...
const yearGan = apiResult.year[0];
const yearJi = apiResult.year[1];
const monthGan = apiResult.month[0];
const monthJi = apiResult.month[1];
const dayGan = apiResult.day[0];
const dayJi = apiResult.day[1];
const hourGan = hasHourInfo ? apiResult.hour[0] : null;
const hourJi = hasHourInfo ? apiResult.hour[1] : null;
const saju = {
name: name,
gender: gender,
calendar: calendar,
...
lunar: apiResult.lunar || '',
solarConverted: apiResult.solar_converted || '',
apiInfo: apiResult.info || {},
};
...
const sipseong = sipseongResult.counts;
const sipseongDetailed = sipseongResult.detailedCounts;
...
let ganElement = '';
if (gan && gan !== dayGan) {
ganSipseong = getSipseongFromGanDetailed(dayGan, gan);
...
if (mainGan !== dayGan) {
jiSipseong = getSipseongFromGanDetailed(dayGan, mainGan);
} else {
jiSipseong = '比肩';
}
}
...
<div style="background:${
                c.severity === __STRING_LITERAL_1259__ ? __STRING_LITERAL_1260__ : __STRING_LITERAL_1261__
...
<div style="background:${
                h.severity === __STRING_LITERAL_1268__ ? __STRING_LITERAL_1269__ : __STRING_LITERAL_1270__
              };color:white;padding:4px 12px;border-radius:20px;font-size:0.85em;">
${h.severity}
</div>
</div>
...
const jiEl = JIJI_ELEMENT[yeonun.ji];
chartHTML += `
...
const jiEl = JIJI_ELEMENT[wolun.zhi];
chartHTML += `
...
const targetScroll = itemLeft - scrollWidth / 2 + itemWidth / 2;
wolunScroll.scrollLeft = targetScroll;
}
}
}
}, 50);
